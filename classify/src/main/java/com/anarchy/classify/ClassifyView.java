...
//                        inMainRegion = true;
    ...
    private Dialog initSubDialog() {
        ...
        //此处设置次级别拖动范围
        mSubRecyclerView.post(() -> {
            mSubContainerWidth = mSubRecyclerView.getMeasuredWidth();
            mSubContainerHeight = mSubRecyclerView.getMeasuredHeight();
        });
        return dialog;
    }
    ...
    private void showSubContainer(final int position) {
        if (mSubDialog == null) {
            ...
            mSubDialog.setOnShowListener(dialog -> getLocationAndFixHeight(mSubRecyclerView, mSubLocation));
            mSubDialog.setOnDismissListener(dialog -> {

            });
            mSubDialog.setOnCancelListener(dialog -> mSubCallBack.onDialogCancel(mSubDialog,position));
        }
        mSubDialog.show();
